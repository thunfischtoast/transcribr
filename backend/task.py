...
app.conf.beat_schedule = {
    "cleanup-audio-files-weekly": {
        ...
        "schedule": crontab(
            day_of_week="sunday", hour=2, minute=0
        ),  # Jeden Sonntag um 2 Uhr morgens
...
def submit_transcription(meeting_id: int, audio_path: str) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Submitting transcription for meeting {meeting_id}, audio: {audio_path}"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            transcript_file = os.path.join(
                transcript_dir, f"meeting_{meeting_id}_transcript.txt"
            )
...
def poll_transcription_status(
    job_id: str, meeting_id: int, max_retries: int = 60, retry_delay: int = 30
):
    """
    ...
    """
    logger.info(
        f"Legacy poll_transcription_status called for job {job_id}, meeting {meeting_id}"
    )
    return {
        "status": "completed",
        "job_id": job_id,
        "message": "Direct transcription, no polling needed",
    }


@app.task
def process_completed_transcript(
    meeting_id: int, transcript_text: str
) -> Dict[str, Any]:
    """
